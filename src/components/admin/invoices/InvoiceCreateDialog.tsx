...
export function InvoiceCreateDialog({ open, onClose }: Props) {
  ...
  async function onSubmit(values: FormValues) {
    ...
    if (validItems.length === 0) {
      toast.error('Bitte mindestens eine gültige Position (Beschreibung + Menge) erfassen.');
      return;
    }
    ...
    // Build sanitized items from validItems (clamps NaN, strips UI fields)
    const sanitizedItems = validItems.map((it, i) => ({
      product_id: it.product_id || undefined,
      description: it.description.trim(),
      quantity: Number(it.quantity) || 0,
      unit: it.unit || 'Stück',
      unit_price_gross: Number(it.unit_price_gross) || 0,
    ...
    const finalTotals = calculateInvoiceTotals(sanitizedItems as any, finalVatRate);
                ...
                <div key={field.id} className="bg-gray-50 border border-gray-200 rounded-lg p-3">
                  ...
                  <div className="mt-2 flex items-center gap-2 text-xs">
                    ...
                    {!watchItems[index]?.product_id && (
                      <span className="text-gray-400 italic ml-1">(wirkt sobald ein Produkt gewählt ist)</span>
                    )}
                  </div>
