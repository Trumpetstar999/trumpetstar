...
export function InvoiceCreateDialog({ open, onClose }: Props) {
  ...
  async function onSubmit(values: FormValues) {
    ...
    // Sanitize items: remove id (auto-generated by DB) and price_type (UI-only), empty product_id → undefined
    const sanitizedItems = computedItems.map((it) => {
      const { id: _id, price_type: _pt, ...item } = it as typeof it & { price_type?: string };
      return { ...item, product_id: item.product_id || undefined };
    });
