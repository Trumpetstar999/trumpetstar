...
export function LevelVideoMessageCard({ data }: LevelVideoMessageCardProps) {
  ...
  const handleClick = () => {
    // Navigate to levels tab - the video will need to be found and opened there
    // Store the video ID in sessionStorage so LevelsPage can auto-open it
    sessionStorage.setItem('autoPlayVideoId', data.id);
    navigateToTab('levels');
  };
