            ...
            >
              ...
              <div className="flex-1 min-h-0 flex items-end justify-between gap-2 pb-1 pt-7">
                {weekData.map((day, index) => {
                  ...
                  return (
                    <div key={index} className="flex-1 h-full flex flex-col items-center gap-1.5">
              ...
              <div className="shrink-0 text-center">
                {totalWeekStars === 0 && (
                  <p className="text-white/50 text-xs">{t('widgets.watchVideosForStars')}</p>
                )}
                {totalWeekStars > 0 && totalWeekStars < 7 && (
                  <p className="text-white/60 text-xs">{t('widgets.keepGoing')}</p>
                )}
                {totalWeekStars >= 7 && (
                  <p className="text-reward-gold text-xs font-medium">{t('widgets.fantasticWeek')}</p>
                )}
              </div>
            </motion.div>
          ) : (
            ...
            >
              ...
              <div className="grid grid-cols-7 gap-1 shrink-0">
                {WEEKDAY_LABELS_DE.map(l => (
                  <div key={l} className="text-center text-[10px] text-white/40 font-medium">{l}</div>
                ))}
              </div>
              ...
              {totalMonthStars === 0 && (
                <p className="text-center text-white/50 text-xs shrink-0">{t('widgets.watchVideosForStars')}</p>
              )}
            </motion.div>
          )}
        </AnimatePresence>
